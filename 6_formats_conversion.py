...
class XMLConverter(Converter):
    ...
    def _xml_to_dict(self, element):
        ...
        if not children:
            # Якщо елемент не має дочірніх елементів, повертаємо його тег та текст
            return {element.tag: element.text.strip() if element.text else None}
        else:
            # Якщо елемент має дочірні елементи, рекурсивно обробляємо їх
            result[element.tag] = []
    ...
    def _dict_to_xml(self, data, parent_element):
        ...
        if isinstance(data, dict):
            ...
            for item in data:
                # Вважаємо, що імена елементів у списку є одниною від батьківського тегу
...
def main():
    """ Головна функція, яка запускає демонстрацію. """
    _create_test_files()
